...
A brief overview how Info part of MetalSwap website works.
...
`prices` - gets METAL prices (current, 24h, 48 and 7d ago) used in calculations (see [src/hooks/useBnbPrices.ts](../src/hooks/useBnbPrices.ts))
...
The flow is controlled by [src/state/info/updaters.ts](../src/state/info/updaters.ts). When user navigates through the site more pools and tokens are automatically loaded, (e.g. you click on METAL token and pools for METAL are loaded automatically, if you click on METAL-BTCB then BTCB token will be loaded, etc)
