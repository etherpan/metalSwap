describe('Swap', () => {
  ...
  // This test requires account with some amount of METAL on it
  // Now with random private key it shows Insufficient METAL Balance button
  it.skip('can swap METAL for BUSD', () => {
  ...
  it('should get input and output currency from url params', () => {
    cy.visit('/swap?inputCurrency=0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56&outputCurrency=METAL')
    cy.get('#swap-currency-input #pair').should('contain', 'BUSD')
    cy.get('#swap-currency-output #pair').should('contain', 'METAL')
  })
