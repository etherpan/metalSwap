 ...
 * One test in swap.test.ts requires to have some METAL amount available to test swap confirmation modal
...
class CustomizedBridge extends Eip1193Bridge {
  ...
  async send(...args) {
    ...
    if (method === 'eth_chainId') {
      if (isCallbackForm) {
        return callback(null, { result: '0x5d3eb' })
      }
      return Promise.resolve('0x5d3eb')
    }
    try {
...
Cypress.Commands.overwrite('visit', (original, url, options) => {
  return original(url, {
    ...
    onBeforeLoad(win) {
      ...
      const provider = new JsonRpcProvider('https://api.metalblockchain.org/ext/bc/C/rpc', 381931)
