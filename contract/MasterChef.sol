...
// MasterChef is the master of Met. He can make Met and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Met is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Mets to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Mets distribution occurs.
        uint256 accTokenPerShare;   // Accumulated Mets per share, times 1e12. See below.
    ...
    // The MET TOKEN!
    address public metAddress;
    //Referrer Address
    address public referralAddress;
    // MET tokens created per block.
    ...
    // The block number when MET mining starts.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(metAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(metAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(metAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(metAddress).transfer(_to, _amount);
        }
