...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[381931] === p.contractAddress[381931])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'METAL'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'METAL'))(
