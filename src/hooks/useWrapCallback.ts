...
export default function useWrapCallback(
  ...
  console.log('debug wbnbContract', wbnbContract)
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, {
                    summary: `Wrap ${inputAmount.toSignificant(6)} METAL to WMETAL`,
                    type: 'wrap',
                  })
                } catch (error) {
            ...
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient METAL balance'),
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WMETAL to METAL` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient WMETAL balance'),
      }
    }
