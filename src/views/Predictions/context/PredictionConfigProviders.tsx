...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.METAL, PredictionSupportedSymbol.CAKE].includes(initToken)) {
      return initToken
    }
  ...
  useEffect(() => {
    ...
    if ([PredictionSupportedSymbol.METAL, PredictionSupportedSymbol.CAKE].includes(upperToken)) {
