...
// NFT WMETAL in testnet contract is different
...
const BuyModal: React.FC<BuyModalProps> = ({ nftToBuy, onDismiss }) => {
  ...
  const [paymentCurrency, setPaymentCurrency] = useState<PaymentCurrency>(PaymentCurrency.METAL)
  ...
  const wbnbAddress = chainId === ChainId.BSC_TESTNET ? TESTNET_WCLD_NFT_ADDRESS : bscTokens.wbnb.address
  ...
  // METAL - returns ethers.BigNumber
  ...
  const walletBalance = paymentCurrency === PaymentCurrency.METAL ? formattedBnbBalance : formattedWbnbBalance
  const walletFetchStatus = paymentCurrency === PaymentCurrency.METAL ? bnbFetchStatus : wbnbFetchStatus

  const notEnoughBnbForPurchase =
    paymentCurrency === PaymentCurrency.METAL
  ...
  useEffect(() => {
    if (bnbBalance.lt(nftPriceWei) && wbnbBalance.gte(ethersToBigNumber(nftPriceWei)) && !isPaymentCurrentInitialized) {
      setPaymentCurrency(PaymentCurrency.WMETAL)
  ...
  const continueToNextStage = () => {
    if (paymentCurrency === PaymentCurrency.WMETAL && !isApproved) {
