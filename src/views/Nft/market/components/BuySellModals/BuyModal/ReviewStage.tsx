  ...
  return (
    <>
      <Flex px="24px" pt="24px" flexDirection="column">
        ...
        <BorderedBox>
          ...
          <ButtonMenu
            ...
            variant="subtle"
          >
            <ButtonMenuItem>METAL</ButtonMenuItem>
            <ButtonMenuItem>WMETAL</ButtonMenuItem>
          </ButtonMenu>
          <Text small color="textSubtle">
            {t('Total payment')}
          </Text>
          ...
          <Text small color="textSubtle">
            {t('%symbol% in wallet', { symbol: paymentCurrency === PaymentCurrency.METAL ? 'METAL' : 'WMETAL' })}
          </Text>
          {!account ? (
          ...
          <Message p="8px" variant="danger">
            <Text>
              {t('Not enough %symbol% to purchase this NFT', {
                symbol: paymentCurrency === PaymentCurrency.METAL ? 'METAL' : 'WMETAL',
              })}
            </Text>
          </Message>
        )}
        <Flex alignItems="center">
          <Text my="16px" mr="4px">
            {t('Convert between METAL and WMETAL for free')}:
          </Text>
          <Button
            as={Link}
            p="0px"
            ...
            external
            variant="text"
            href="/swap?inputCurrency=METAL&outputCurrency=0x479E0638F61ab12c6D1a947CD667d522bC69910d"
          >
            {t('Convert')}
          </Button>
        </Flex>
      ...
      <Flex px="24px" pb="24px" flexDirection="column">
        ...
        <Button as={Link} external style={{ width: '100%' }} href="/swap?outputCurrency=METAL" variant="secondary">
          {t('Get %symbol1% or %symbol2%', { symbol1: 'METAL', symbol2: 'WMETAL' })}
        </Button>
      </Flex>
    </>
  )
