...
/**
 * Price data for token and metal based on block number
 */
...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    // format token METAL price results
    const tokenPrices: {
      timestamp: string
      derivedBNB: number
      priceUSD: number
    ...
    // Get Token prices in METAL
    Object.keys(prices).forEach((priceKey) => {
      const timestamp = priceKey.split('t')[1]
      // if its METAL price e.g. `b123` split('t')[1] will be undefined and skip METAL price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
    ...
    // Go through METAL USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
