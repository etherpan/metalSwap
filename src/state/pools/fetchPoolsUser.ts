...
// METAL pools use the native METAL token (wrapping ? unwrapping is done at the contract level)
const nonBnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol !== 'METAL')
const bnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol === 'METAL')
