...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://metswap.finance - check the URL carefully.")
    return warningText.split(/(https:\/\/metswap.finance)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://metswap.finance'}
          color={text === 'https://metswap.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
